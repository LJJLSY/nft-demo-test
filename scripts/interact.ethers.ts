...
async function main() { 
    ...
    console.log("铸造ID为1的NFT...");
    const tx1NFT = await NFT_seller.mint(seller_Wallet.address, 1,{nonce:currentNonce});
    ...
    console.log("授权给拍卖合约...");
    const tx2NFT = await NFT_seller.approve(auction_seller.getAddress(), 1, {nonce:currentNonce});
    ...
    const tx3 = await auction_bidder.bid(bidauctionid, bidamount, { value: bidamount });
    ...
    //获取当前链上的最新区块时间戳
    const latestBlock = await networkconnect.ethers.provider.getBlock("latest");
    const currentTimestamp = latestBlock ? latestBlock.timestamp : Math.floor(Date.now() / 1000);
    //计算出未来时间戳，并加缓冲确保时间到
    const auctionEndTime = currentTimestamp + duration + 10;
    ...
    const verifyBlock = await networkconnect.ethers.provider.getBlock("latest");
    console.log("挖矿后链上时间戳：",verifyBlock!.timestamp);
    ...
    const tx4 = await auction_admin.end(endauctionid);
